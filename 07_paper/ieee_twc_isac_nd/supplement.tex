...
\begin{table*}[!t]
  ...
  \begin{tabular}{p{0.17\textwidth}p{0.12\textwidth}p{0.38\textwidth}p{0.16\textwidth}}
    ...
    Baselines & Covered & Uniform random, SkyOrbs-like, learned policy without ISAC, enhanced learned policy without ISAC, enhanced learned policy with ISAC. & SkyOrbs-like is not a full SkyOrbs reproduction. \\
...
\begin{figure*}[!t]
  ...
  \caption{Training-history evidence for the shared-parameter policy-search run. Left: reward evolution. Right: selection score evolution. These curves support empirical stabilization only, not theoretical convergence.}
...
\begin{figure*}[!t]
  ...
  \caption{Range and timing sensitivity. Left: discovery gain over the no-ISAC counterpart as $\Rc/D$ and $\Rs/\Rc$ vary. Right: discovery rate under slot durations from 1 ms to 20 ms.}
...
\begin{table*}[!t]
  ...
  \begin{tabular}{p{0.20\textwidth}ccp{0.52\textwidth}}
    ...
    \midrule
    \texttt{main} & 98 & 294 & Manuscript tables, range, error, and ablation claims. \\
    \texttt{main\_boundary} & 24 & 72 & Mobility-boundary evidence in the main paper. \\
    \texttt{supplement} & 111 & 318 & Scale, beamwidth, mobility, and error extensions. \\
