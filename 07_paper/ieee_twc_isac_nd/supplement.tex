...
To test whether the B=15 collision-penalized boundary is inherent to ISAC candidate refinement or caused by role selection, we add a lightweight distributed variant, \texttt{collision\_aware\_isac}.
...
In the same five-seed paired setting, the collision-aware variant obtains B=10 discovery 0.3697 and collision-penalized discovery 0.3162, compared with 0.3639 and 0.2995 for the proposed low-latency variant.
At B=15, it obtains discovery 0.5696 and collision-penalized discovery 0.2504, compared with 0.5445 and 0.2042 for the proposed low-latency variant and 0.5107 and 0.2207 for the one-slot delayed variant.
The collision-penalized delta is positive in all 5/5 paired seeds versus both the proposed low-latency protocol and the one-slot delayed variant for B=10 and B=15.
This probe does not close the full MAC-design problem; it shows that the collision boundary identified in Fig.~\ref{fig:supp_round11_stability} is addressable by distributed role-control refinement.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.48\textwidth]{../../06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_collision_penalized.png}
  \hfill
  \includegraphics[width=0.48\textwidth]{../../06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_collision_count.png}
  \caption{Collision-aware MAC refinement probe in the same N=100 five-seed paired setting. Left: collision-penalized discovery. Right: collision count. The local role-control refinement improves collision-penalized discovery while preserving the same ISAC candidate-set handshake interface.}
  \label{fig:supp_round12_collision_aware}
\end{figure*}
...
Under this model, the collision-aware variant improves discoveries per joule from 6.1722 to 6.6080 at B=10 and from 9.2453 to 10.2430 at B=15 relative to the proposed low-latency variant.
The corresponding energy per discovered edge decreases from 0.1620 J to 0.1515 J at B=10 and from 0.1082 J to 0.0977 J at B=15.
Fig.~\ref{fig:supp_round12_energy} reports these two accounting metrics.

\begin{figure*}[!t]
  \centering
  \includegraphics[width=0.48\textwidth]{../../06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_discoveries_per_joule.png}
  \hfill
  \includegraphics[width=0.48\textwidth]{../../06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_energy_per_discovery.png}
  \caption{Assumed radio-state energy accounting in the round12 probe. Left: discoveries per joule. Right: energy per discovered edge. These are diagnostic accounting metrics under stated default radio-state powers, not platform-calibrated energy-optimality results.}
  \label{fig:supp_round12_energy}
\end{figure*}
...
The round12 collision-aware MAC refinement endpoints are also archived separately as a mechanism-refinement probe, not as a replacement for the main protocol table.
