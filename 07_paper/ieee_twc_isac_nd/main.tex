...
This result is also a modeling caveat: the candidate-set refinement assumes that ISAC-updated candidate beams can affect readiness inside the discovery opportunity represented by a slot.
A hardware-constrained implementation may need to delay the candidate-set use by one slot or restrict the number of candidate beams, which should be evaluated in future work.
...
Fourth, candidate-set refinement currently abstracts sub-slot or readiness behavior that requires a dedicated implementation analysis.
Fifth, extremely narrow beams at 3 and 5 degrees remain a stress region and should be discussed as such.
Finally, the proposed ISAC-assisted policy creates more collision attempts than the no-ISAC baseline in dense finite-horizon tests; collision-normalized efficiency and energy cost should be added before a submission-ready version.
