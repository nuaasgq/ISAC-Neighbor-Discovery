...
规则协议：ISAC-Assisted Topology-Aware Probabilistic Neighbor Discovery，简称 `I-TAP-ND`。

学习增强协议：MARL-enhanced I-TAP-ND，简称 `MARL-I-TAP-ND`。

## 设计目标

在完全分布式、无中心、自身定位姿态可用但对准前未知邻居状态的 UAV-UAV 三维窄波束网络中，利用 ISAC 生成的 beam-cell occupancy prior 减少空扫，并通过 topology-aware priority 与 MARL 策略在有限时间内优先发现更有价值的链路。
...
6. MARL actor 执行期只能使用本地观测；训练期 critic 可使用仿真全局信息。

## 节点状态

每个节点维护：

- `Self_i`：自身位置、速度、航向和姿态。
...
MARL 版本不替代握手机制和 ISAC belief 更新规则，而是学习 mode/beam 调度：

```text
a_i(t) ~ pi_theta(a | o_i(t))
```

其中 `pi_theta` 为所有 UAV 共享的 actor，执行期仅使用本地观测：

- 自身位置、速度、航向和姿态。
- ISAC beam-cell prior、置信度、staleness、粗距离/多普勒。
- 已发现邻居集合的 pooled summary。
- 本地发现历史、碰撞历史和动作历史。
...
- MAPPO 或 IPPO 作为首选实现。
- 参数共享 actor。
- beam-cell encoder + neighbor set pooling + GRU memory。
- 规则 priority 作为 logits bias 或 policy input。

MARL 输出：

```text
pi(a_i) = pi_mode(mode | o_i)
        * pi_beam(beam | mode, o_i)
```

后续可扩展功率、sensing budget 和可变波宽。
