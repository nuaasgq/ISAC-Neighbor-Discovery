...
| B=15 error robustness appears strong under Gauss-Markov but more sensitive under random-walk mobility. | `06_analysis/paper_tables/round8_error_profiles_b15_gm_rw_quick/aggregate_metrics.csv`; one-seed sanity result with full ISAC far above no-ISAC, but random-walk discovery declines from 0.4503 to 0.2240 as configured errors increase. | Treat as quick sanity evidence until the full three-seed B=15 job finishes. |
