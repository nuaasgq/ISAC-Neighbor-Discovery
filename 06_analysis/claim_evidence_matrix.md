...
| The proposed policy shows observed improvement over random, SkyOrbs-like, learned no-ISAC, and enhanced no-ISAC baselines in the main large-scale 10-degree comparison at `N=100`, density scaling. | `06_analysis/paper_tables/round3_robustness/n100_density_multiseed/aggregate_metrics.csv`; main table added in `main.tex` with discovery 0.3655 versus near-zero baselines. | Main baseline comparison table. |
| The main comparison remains positive under paired seed-level deltas. | `06_analysis/paper_tables/paired_delta_summary/paired_delta_summary.csv`; N=100/B=10 proposed vs enhanced no-ISAC has paired discovery delta +0.3648 and lambda2 delta +12.92 across 3/3 positive seeds. | Reviewer-facing statistical stability support; bootstrap CIs are descriptive because seed count is small. |
...
| Local collision-aware role control can mitigate the B=15 collision-penalized boundary without changing the ISAC candidate-set interface. | `06_analysis/paper_tables/round12_collision_aware_probe_v2/`; collision-aware collision-penalized discovery is 0.3162 vs proposed 0.2995 at B=10 and 0.2504 vs 0.2042 at B=15, with 5/5 positive paired deltas versus proposed and one-slot delay at both beamwidths. | Mechanism-refinement evidence; do not present as a complete MAC or energy optimization. |
...
| Energy efficiency is not yet Joule-level. | No radio-state energy model has been implemented; round12 improves collision-normalized discovery but not radio-state energy accounting. | Report scan-action and collision-normalized efficiency only. |
