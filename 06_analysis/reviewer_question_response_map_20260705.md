...
|---|---|---|---|
| Is this a physical-layer ISAC beamforming paper? | No. ISAC is abstracted as an imperfect beam-cell occupancy-prior service exposed to the link layer, with cite-backed mapping to sensing-service KPIs and communication-link support. | `main.tex` System Model and ISAC Prior; `supplement.tex` Range and Time-Scale Sensitivity plus PHY-to-protocol table. | No waveform, detector, or radar-equation calibration is claimed. |
...
| How is the SkyOrbs-like baseline scoped? | The paper does not claim a strict head-to-head win over complete SkyOrbs. It includes a SkyOrbs-like 3-D skip-scan baseline under the same information boundary, with a supplement note and standalone scope appendix documenting what is and is not matched. | `main.tex` Baselines and Limitations; `supplement.tex` Baseline Scope Note; `06_analysis/skyorbs_like_baseline_scope_appendix_20260705.md`; `round8` and `round9` full-baseline figures. | Complete SkyOrbs reproduction is future work. |
...
| Does `Rs > Rc` physically stop helping? | No physical range law is claimed. `Rs` is sensing-observation support and `Rc` is communication-handshake support; marginal protocol gain saturates because only communication-range neighbors can be confirmed as links. | Range grid figure; ISAC abstraction paragraph; PHY-to-protocol mapping table. | `Rs`, error rates, angular offsets, and staleness are protocol abstraction parameters unless a later calibration appendix is added. |
...
| Are collisions ignored? | No. Collision count and collision-penalized discovery are reported; dense wider beams can improve raw discovery while increasing collisions. Round13 adds a ten-seed local collision-aware role-control probe that improves collision-penalized discovery at B=10/B=15. | Main limitations; round11/round13 collision-penalized supplement figures. | Current method is still not a complete collision- and energy-aware MAC optimization. |
| Are the main N=100/B=10/B=15 results seed-stable? | Round14 provides a ten-seed main-table check at N=100/B=10 with 10/10 positive discovery deltas versus random, SkyOrbs-like, learned no-ISAC, and enhanced no-ISAC controls. Round11 provides focused B=10/B=15 raw-discovery stability, while round13 provides a ten-seed collision/energy refinement probe. | `round14_main_table_10seed_n100_b10`, `round11_paired_seed_campaign_main`, and `round13_collision_energy_10seed` endpoint, cumulative, and paired-delta tables/figures. | Use each campaign for its scope; round13 is a refinement probe, not a replacement for the full main table. |
| Does higher discovery mean MAC efficiency is solved? | No. Round11 shows proposed raw discovery is highest, but B=15 collision-penalized discovery can favor the one-slot-delay variant. Round13 then shows the boundary can be mitigated by collision-aware local role control, with collision-penalized and energy-accounting gains positive in 10/10 paired seeds. | `round11_collision_penalized.png`; `round13_collision_penalized.png`; round11/round13 paired tables. | Role control is promising, but full MAC and energy scheduling remain next-stage protocol problems. |
...
- "The implemented SkyOrbs-like baseline should not be interpreted as a full reproduction..."
