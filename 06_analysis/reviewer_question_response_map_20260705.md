...
| Are the main N=100/B=10/B=15 results seed-stable? | Round11 provides focused raw-discovery stability evidence, while round13 provides a ten-seed collision/energy refinement probe. The round11 raw-discovery deltas are 5/5 positive against random, enhanced no-ISAC, candidate-set removal, and one-slot delay at both beamwidths. | `round11_paired_seed_campaign_main` and `round13_collision_energy_10seed` endpoint, cumulative, and paired-delta tables/figures. | Use each campaign for its scope; round13 is a refinement probe, not a replacement for the full main table. |
