...
| Is this a full MARL contribution? | No. The current learning evidence is shared-parameter policy optimization, used to tune scalable local decision rules. | `main.tex` Shared-Parameter Policy Optimization; training curves. | MAPPO/QMIX/GNN MARL are supported by the environment but are not main evidence. |
...
| Are collisions ignored? | No. Collision count and collision-penalized discovery are reported; dense wider beams can improve raw discovery while increasing collisions. | Main limitations; collision-penalized supplement figures. | Current method is not a complete collision-aware MAC optimization. |
