...
|---|---|---|---|
| The learning method is not full neural MARL. | The project motivation includes multi-agent RL, but the strongest current evidence is shared-parameter/CEM-style protocol tuning. | `main.tex` explicitly says neural actor-critic/value-decomposition variants are not used as main performance evidence; actor-critic probes are documented separately. | Use `shared-parameter protocol tuning`, not full MAPPO/QMIX/GNN-MARL. |
| SkyOrbs is not strictly reproduced. | The baseline is inspired by SkyOrbs but not the full original protocol. | `skyorbs_baseline_reproduction_checklist.md`; `skyorbs_like_baseline_scope_appendix_20260705.md`; supplement baseline-scope note; baseline text says `SkyOrbs-like`. | Never claim reproduction. Use it as a deterministic 3-D skip-scan reference baseline. |
| Collision cost is high for the proposed policy. | Wider beams and ISAC-guided concentration increase collision attempts in dense settings. | Tables include collision counts; round7/round8 include collision-penalized discovery; round13 adds a ten-seed local collision-aware role-control probe that improves collision-penalized discovery at B=10/B=15. | Report raw discovery with collision-aware metrics. State that collision-aware role control mitigates but does not complete MAC/energy optimization. |
...
| ISAC model is abstract. | TWC/TCOM reviewers may expect physical-layer sensing equations. | System model now separates sensing-service support from communication-link support using 3GPP citations, and the supplement maps `Rc`, `Rs`, false alarms, misses, angular offsets, and staleness to PHY/service interpretations. | Keep physical waveform/estimator outside scope; do not imply calibrated hardware-ready sensing. |

## P2 Risks

| Risk | Why a reviewer may object | Current mitigation | Manuscript posture |
|---|---|---|---|
| Tables only show means. | Statistical stability may be questioned. | `statistical_stability_summary.csv` contains mean/std/CI; Table IV is now a ten-seed main-table check, while other compact main tables remain mean-focused with supplement support. | Mention std/CI in supplement/reproducibility tables. |
...
| Training convergence may be overclaimed. | CEM score curve is empirical and finite, with points generated after candidate-policy evaluations. | Section now uses cumulative training steps on the x-axis and explicitly says each point is a full candidate-policy evaluation. | Do not use `convergence proof` or per-gradient MARL convergence language. |
