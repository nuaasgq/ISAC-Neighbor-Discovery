...
| Collision-aware MAC refinement | Covered as mechanism probe | `06_analysis/paper_tables/round12_collision_aware_probe_v2/`; `06_analysis/paper_figures/round12_collision_aware_probe_v2/`. | Shows the B=15 collision-penalized boundary can be mitigated by local role control; includes assumed radio-state accounting but not platform-calibrated energy-aware MAC optimization. |
...
| More than 16 paper-ready figures, 4:3 aspect, Times-style fonts | Covered | `06_analysis/paper_figures` archives a large figure pool; round11 and round12 selected figures are 1920 x 1440. Figure manifests document 4:3 and Times New Roman with serif fallback. | The final manuscript should select a small subset for the main text and use the rest in supplement. |
...
| Can collision-aware role control mitigate the B=15 boundary? | `06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_collision_penalized.png`; `round12_collision_count.png`; `round12_collision_penalized_delta.png`. |
| What happens under assumed radio-state energy accounting? | `06_analysis/paper_figures/round12_collision_aware_probe_v2/round12_discoveries_per_joule.png`; `round12_energy_per_discovery.png`. |
...
- Assumed radio-state energy accounting is now available in round12, but platform-calibrated delay-power optimization is not.
...
- Round11 shows B=15 raw discovery remains strongest for the proposed protocol, but one-slot delay can be better on collision-penalized discovery. Round12 shows a local collision-aware role-control probe can mitigate this boundary, but full MAC/energy optimization remains open.
