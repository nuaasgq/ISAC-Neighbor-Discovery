...
| Train at 10-degree beams, test other beamwidths | Covered | Main 5/10/15/30 degree transfer: `round3_robustness/n100_*_multiseed`; stress 3/5/10/15/30 degree transfer: `round7_scale_beam_grid_light`. | Strongest evidence is 10--30 degrees; 3/5 degrees show the hard lower-beamwidth boundary. |
...
| RL without ISAC baseline | Covered | `rl_no_isac` rows in `round3_robustness/n100_*_multiseed` and `round8_n100_multimobility_full_baseline/combined_aggregate_metrics.csv`. | Used to isolate communication-only learning; do not call the main method full neural MARL. |
| Improved RL without ISAC baseline | Covered | `improved_rl_no_isac` rows in main and supplement tables. | Shows that rule/policy improvement without sensing remains near zero in large narrow-beam settings. |
| Improved RL plus ISAC | Covered | `improved_rl_isac` rows in all main and supplement tables. | Main performance evidence for the proposed protocol. |
...
| More than 16 paper-ready figures, 4:3 aspect, Times-style fonts | Covered | `06_analysis/paper_figures` contains 354 PNG figures after round8 full-baseline additions; new full-baseline figures are 1920 x 1440. Figure manifests document 4:3 and Times New Roman with serif fallback. | The final manuscript should select a small subset for the main text and use the rest in supplement. |
| Network-structure or method innovation beyond blind search | Partially covered | Mechanism figures, candidate-set refinement ablation, topology-aware scoring ablation, one-slot delay ablation. | Current strongest innovation is cross-layer ISAC occupancy prior plus topology-aware link-layer discovery. Neural MARL architecture innovation remains a future or secondary thread. |
