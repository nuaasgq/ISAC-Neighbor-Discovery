...
| Learned policy without ISAC baseline | Covered | `rl_no_isac` rows in `round3_robustness/n100_*_multiseed` and `round8_n100_multimobility_full_baseline/combined_aggregate_metrics.csv`. | Used to isolate communication-only shared-policy learning; do not call the main method full neural MARL. |
...
| Network-structure or method innovation beyond blind search | Partially covered | Mechanism figures, candidate-set refinement ablation, topology-aware scoring ablation, one-slot delay ablation. | Current clearest innovation is cross-layer ISAC occupancy prior plus topology-aware link-layer discovery. Neural MARL architecture innovation remains a future or secondary thread. |
