...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build round12 collision-aware MAC refinement evidence.")
...
def build_paired_delta_table(episode, pd):
    ...
    controls = ("improved_rl_isac", "ablation_isac_one_slot_delay", "improved_rl_no_isac", "uniform_random")
    metrics = (
        "discovery_rate",
        "collision_penalized_discovery_rate",
        "collision_count",
    ...
    for beamwidth, beam_df in episode.groupby("beamwidth_deg"):
        treatment = beam_df[beam_df["protocol"] == "collision_aware_isac"]
                ...
                rows.append(
                    {
                        ...
                        "treatment": "collision_aware_isac",
                        "control": control,
                        "metric": metric,
...
def plot_paired_delta(paired, path: Path, plt) -> dict:
    ...
    rows = paired[(paired["metric"] == "collision_penalized_discovery_rate") & (paired["control"].isin(controls))]
    ...
    ax.set_ylabel("Collision-penalized delta")
    ax.legend(frameon=False)
    fig.tight_layout()
    fig.savefig(path)
    plt.close(fig)
    return {"path": str(path), "type": "paired_delta", "metric": "collision_penalized_discovery_rate"}
