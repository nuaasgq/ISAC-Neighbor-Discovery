...
`s_i^self(t)` 包含自身位置、速度和姿态。`O_i(t)` 是 ISAC 观测诱导的 beam-cell occupancy prior，不包含未发现邻居 ID 或真实位置。
...
其中 `discovered_neighbor_summary_i(t)` 只能来自已握手邻居。
...
H5：参数共享 MARL actor 在 `N_train` 小规模场景训练后，可在 `N_test > N_train` 的大规模场景中零微调部署，并保持相对启发式基线的正收益。
